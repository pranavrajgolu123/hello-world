...
public class UpdateNetworkingDetailV6Popup extends CommonPopUp
{
	...
	public UpdateNetworkingDetailV6Popup( CreateNetworkModel networkModel, UIComponentsUtil uiComponentsUtil, Icommand updateNetworkDetailCmd, IPType ipType )
	{
	...
	private void initialize()
	{
		...
		dnsAddIcon.addClickHandler( new ClickHandler()
		{
			...
			public void onClick( ClickEvent event )
			{
				addNewDnsAndDnsSearchBox( null, null );
			}
		} );
		...
		if ( !networkModel.getDhcp6() )
		{
			...
			addressAddIcon.setEnabled( true );
			addressesPanel.setVisible( true );
			gateWay.setText( networkModel.getGateway6() );
		...
		if ( !networkModel.getDhcp6() /*&& ( ( networkModel.getDns_search() != null && !networkModel.getDns_search().isEmpty() ) || ( networkModel.getDns_addresses() != null && !networkModel.getDns_addresses().isEmpty() ) ) */ )
		{
			...
			for ( int i = 0; i < networkModel.getDns_search().size(); i++ )
			{
				if ( i == 0 )
				{
					...
					if ( networkModel.getDns_addresses().size() >= i && networkModel.getDns_addresses().get( i ) != null )
					{
				...
				else
				{
					...
					if ( networkModel.getDns_addresses().size() >= i && networkModel.getDns_addresses().get( i ) != null )
					{
						dnsAddress = networkModel.getDns_search().get( i );
					}
					addNewDnsAndDnsSearchBox( networkModel.getDns_addresses().get( i ), dnsAddress );
				}
			}
	...
	private void initializeAddressPanel()
	{
		...
		gateWay = uiComponentsUtil.getIP6TextBox( "Gateway", "", "s12", false );
		gateWay.addStyleName( resources.style().updateNetworkGateway() );
		firstIP = uiComponentsUtil.getIP6TextBox( "IP", "", "s10", false );
		firstIP.addStyleName( resources.style().updateNetworkIp() );
		firstSubnet = uiComponentsUtil.getIP6SubnetBox( "Subnet Prefix", "", "s2", false, firstIP );
	...
	private void initializeDnsPanel()
	{
		...
		firstDns = uiComponentsUtil.getIP6TextBox( "Server", "", "s11", false );
	...
	private MaterialPanel getIPBoxPanel( String ip, String subNet )
	{
		...
		ipTextBox = uiComponentsUtil.getIP6TextBox( "IP", "", "s8", false );
		ipTextBox.addStyleName( resources.style().updateNetworkIp() );
		subnet = uiComponentsUtil.getIP6SubnetBox( "Subnet prefix", "", "s3", false, ipTextBox );
		...
		addButtonClickCommand( new Icommand()
		{
			...
			public void execute()
			{
				if ( /* validate() */ true )
				{
					...
					networkModel.setDhcp6( addressesSwitch.getValue() );
					if ( !addressesSwitch.getValue() )
					{

						networkModel.setGateway6( gateWay.getValue() );

						networkModel.setAddresses( getAddresses() );
					}
					else
					{
						networkModel.setGateway6( "" );
	...
	private List<String> getAddresses()
	{
		...
		if ( ipList != null && !ipList.isEmpty() )
		{
			...
			{
				// Here there is no validation so we need to check manually the values 
				String address = null;
				...
				{
					address = "/" + subnetList.get( i ).getValue();
				}
				if ( address != null )
				{
	...
	{
		List<String> dnsSearchDomains = new ArrayList<>();
		if ( dnsSearchList != null && !dnsSearchList.isEmpty() )
		{
			for ( MaterialTextBox ipBox : dnsSearchList )
			{
				if ( ipBox.getValue() != null && !ipBox.getValue().trim().isEmpty() )
				{
					...
					if ( address != null )
					{
						dnsSearchDomains.add( ipBox.getValue() );
					}
				}
			}
		}
	...
	private void addNewDnsAndDnsSearchBox( String dnsName, String dnsSearchName )
	{
